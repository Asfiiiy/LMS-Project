...
const ForumPageContent = () => {
  ...
  const fetchSidebarData = async () => {
    try {
      ...
      // Lightweight stats — single DB query, no post payload
      const statsRes = await apiService.getForumStats();
      if (statsRes?.success) {
        setForumStats({
          totalPosts: statsRes.totalPosts ?? 0,
          totalComments: statsRes.totalComments ?? 0,
          totalUsers: statsRes.totalMembers ?? 0,
          activeToday: statsRes.activeToday ?? 0
        });
        // Trending from stats (id, title, likes_count, comments_count)
        const trending = (statsRes.trending || []).map((t: { id: number; title: string; likes_count?: number; comments_count?: number; count?: number }) => ({
          id: t.id,
          title: t.title,
          likes_count: t.likes_count ?? t.count ?? 0,
      ...
      // User stats from stats response (when authenticated)
      if (statsRes?.userStats) {
        setUserStats(statsRes.userStats);
      }
    } catch (error) {
    } finally {
  ...
  const canPost = ['Student', 'Moderator', 'Admin', 'Assessor', 'ManagerStudent', 'InstituteStudent'].includes(userRole || '');
  ...
  return (
    <ProtectedRoute allowedRoles={['Student', 'Moderator', 'Admin', 'Assessor', 'ManagerStudent', 'InstituteStudent']} userRole={userRole}>
