...
const ManageQualificationCourse = () => {
  ...
  // Rule Level 3 settings
  const [ruleLevel3Enabled, setRuleLevel3Enabled] = useState(false);
  const [ruleLevel3RequiredUnits, setRuleLevel3RequiredUnits] = useState(0);
  const [ruleLevel3SelectableUnits, setRuleLevel3SelectableUnits] = useState(0);
  const [savingRuleLevel3, setSavingRuleLevel3] = useState(false);
  ...
  const loadCourseData = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Load Rule Level 3 settings
        if (response.course?.rule_level_3_enabled !== undefined) {
          setRuleLevel3Enabled(response.course.rule_level_3_enabled || false);
          setRuleLevel3RequiredUnits(response.course.rule_level_3_required_units || 0);
          setRuleLevel3SelectableUnits(response.course.rule_level_3_selectable_units || 0);
        }
      } else {
  ...
  const handleAddUnit = async () => {
    ...
    try {
      ...
      formData.append('rule_level_3_enabled', newUnit.rule_level_3_enabled.toString());
  ...
  // Drag and Drop Handlers for Units
  const handleDragStart = (e: React.DragEvent, unitId: number) => {
    setDraggedUnit(unitId);
    e.dataTransfer.effectAllowed = 'move';
  };

  const handleDragOver = (e: React.DragEvent, unitId: number) => {
    e.preventDefault();
  ...
  const handleDrop = async (e: React.DragEvent, dropTargetId: number) => {
    ...
    if (!draggedUnit || draggedUnit === dropTargetId) {
      setDraggedUnit(null);
      setDragOverUnit(null);
      return;
    }

    // Create new units array with updated order
    const sortedUnits = [...units].sort((a, b) => a.unit_number - b.unit_number);
    const draggedIndex = sortedUnits.findIndex(u => u.id === draggedUnit);
    const dropIndex = sortedUnits.findIndex(u => u.id === dropTargetId);

    if (draggedIndex === -1 || dropIndex === -1) return;

    // Reorder units
    const [removed] = sortedUnits.splice(draggedIndex, 1);
    sortedUnits.splice(dropIndex, 0, removed);

    // Update unit_number for all units
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/qualification/units/reorder`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          course_id: courseId,
          units: updatedUnits.map(u => ({ id: u.id, unit_number: u.unit_number }))
        })
      });
      ...
      if (!data.success) {
        showToast('Failed to save unit order', 'error');
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-6 border-t-4 border-purple-600">
            ...
            <div className="mt-6 p-6 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl border-2 border-yellow-200">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Enable this feature to allow students to choose which units to complete after finishing required units.
              </p>
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-3 p-4 bg-white rounded-lg border border-yellow-200">
                  <input
                    type="checkbox"
                    id="ruleLevel3Enabled"
                    checked={ruleLevel3Enabled}
                    onChange={(e) => setRuleLevel3Enabled(e.target.checked)}
                    className="w-5 h-5 text-yellow-600 rounded focus:ring-2 focus:ring-yellow-500"
                  />
                  <label htmlFor="ruleLevel3Enabled" className="text-sm font-semibold text-gray-700 cursor-pointer">
                    Enable Rule Level 3 (Unit Selection Feature)
                  </label>
                </div>
                
                {ruleLevel3Enabled && (
                  <div className="grid grid-cols-2 gap-4 p-4 bg-white rounded-lg border border-yellow-200">
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">
                        Required Units to Complete <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="number"
                        min="1"
                        max={units.length}
                        value={ruleLevel3RequiredUnits}
                        onChange={(e) => setRuleLevel3RequiredUnits(parseInt(e.target.value) || 0)}
                        placeholder="e.g., 4"
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        Number of units students must complete before they can choose
                      </p>
                    </div>
                    
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">
                        Selectable Units <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="number"
                        min="1"
                        max={units.length - ruleLevel3RequiredUnits}
                        value={ruleLevel3SelectableUnits}
                        onChange={(e) => setRuleLevel3SelectableUnits(parseInt(e.target.value) || 0)}
                        placeholder="e.g., 2"
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
                      />
                      <p className="text-xs text-gray-500 mt-1">
                        How many units students can choose from the remaining units
                      </p>
                    </div>
                  </div>
                )}
                ...
                {ruleLevel3Enabled && (
                  <button
                    onClick={async () => {
                      if (!ruleLevel3RequiredUnits || !ruleLevel3SelectableUnits) {
                        showToast('Please fill in all required fields', 'warning');
                        return;
                      }
                      if (ruleLevel3RequiredUnits + ruleLevel3SelectableUnits > units.length) {
                        showToast('Required + Selectable units cannot exceed total units', 'warning');
                        return;
                      }
                      
                      try {
                        setSavingRuleLevel3(true);
                        const response = await apiService.updateQualificationRuleLevel3(
                          courseId,
                          ruleLevel3Enabled,
                          ruleLevel3RequiredUnits,
                          ruleLevel3SelectableUnits
                        );
                        
                        if (response.success) {
                          showToast('Rule Level 3 settings saved successfully!', 'success');
                          loadCourseData();
                        } else {
                        ...
                        setSavingRuleLevel3(false);
                      }
                    }}
                    disabled={savingRuleLevel3}
                    className={`px-6 py-2 bg-yellow-600 text-white rounded-lg transition-colors font-semibold flex items-center gap-2 ${
                      savingRuleLevel3 
                  ...
                  >
                    {savingRuleLevel3 ? (
                      ...
                      <>
                        <span>💾</span>
                        <span>Save Rule Level 3 Settings</span>
                      </>
                    )}
                  </button>
                )}
              ...
              <div className="mb-6 p-6 bg-purple-50 rounded-xl border-2 border-purple-200">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center space-x-2 p-4 bg-yellow-50 rounded-lg border-2 border-yellow-200">
                      <input
                        type="checkbox"
                        id="ruleLevel3"
                        checked={newUnit.rule_level_3_enabled}
                        onChange={(e) => setNewUnit({...newUnit, rule_level_3_enabled: e.target.checked})}
                        className="w-5 h-5 text-yellow-600 rounded focus:ring-2 focus:ring-yellow-500"
                      />
                      <label htmlFor="ruleLevel3" className="text-sm font-semibold text-gray-700 cursor-pointer">
                        🎯 Enable Rule Level 3 (Student can choose this unit after completing required units)
                      </label>
                    </div>
                  </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <select
                              ...
                              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                            >
                              <option value="pass_fail">Pass/Refer</option>
            ...
            {units.length === 0 ? (
              ...
              <div className="space-y-4">
                {units.map((unit, index) => (
                  <div
                    key={unit.id}
                    className={`border-2 rounded-xl p-6 hover:border-purple-300 transition-all cursor-pointer bg-gradient-to-r from-white to-purple-50 ${
                      dragOverUnit === unit.id ? 'border-purple-500 border-4 bg-purple-100' : 'border-gray-200'
                    } ${draggedUnit === unit.id ? 'opacity-50' : ''}`}
                    draggable
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1" onClick={() => handleUnitClick(unit.id)}>
                        <h3 className="text-xl font-bold text-gray-900 mb-2 flex items-center gap-2">
                          <span className="text-2xl cursor-move mr-2" title="Drag to reorder">⋮⋮</span>
                          <span className="text-purple-600">{unit.title}</span>
                          {!!unit.is_optional && (
                        ...
                        <div className="flex flex-wrap gap-4 text-sm">
                          <span className="flex items-center gap-1 text-gray-600">
                            🔓 <strong>Unlock:</strong> {unit.unlock_condition ? (unit.unlock_condition === 'none' ? 'Default' : unit.unlock_condition.replace('_', ' ')) : 'Default'}
                          </span>
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-xl p-6">
            ...
            <ul className="space-y-1 text-purple-700 text-sm list-disc list-inside">
              ...
              <li><strong>Grading Types:</strong> Each submission can be graded as Score (0-100) or Pass/Refer</li>
