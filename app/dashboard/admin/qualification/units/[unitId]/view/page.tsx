...
export default function ViewQualificationUnit() {
  ...
  const handleFileClick = (filePath: string, fileName: string, e: React.MouseEvent) => {
    ...
    if (fileExtension === 'pdf') {
      // Mobile browsers don't support PDFs in iframes - open in new window
      if (isMobile) {
            ...
            <div className="bg-white rounded-lg w-11/12 h-5/6 flex flex-col">
              ...
              <div className="flex-1 relative">
                ...
                <iframe
                  ...
                  style={{ border: 'none' }}
                  onLoad={() => {
                    if (process.env.NODE_ENV === 'development') { console.log('[Unit View] PDF loaded successfully'); }
                    setPdfLoading(false);
                  }}
                  onError={() => {
                    if (process.env.NODE_ENV === 'development') { console.error('[Unit View] PDF load error'); }
                    setPdfError(true);
                  }}
                />
              </div>
