...
const AdminCPDView = () => {
  ...
  const [userRole, setUserRole] = useState<'Admin' | 'Assessor' | 'Student' | 'Operation Manager' | 'Team Member' | null>(null);
  ...
  useEffect(() => {
    ...
    if (user && user.id) {
      ...
      setUserRole(role as 'Admin' | 'Assessor' | 'Student' | 'Operation Manager' | 'Team Member' | null);
  ...
  const handleExtendDeadline = async (topicId: number) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/cpd/topics/${topicId}/deadline`, {
        method: 'PUT',
        headers: {
  ...
  const getDownloadUrl = (fileUrl: string, fileName: string) => {
    const apiUrl = getApiUrl();
    return `${apiUrl}/api/cpd/download-file?url=${encodeURIComponent(fileUrl)}&filename=${encodeURIComponent(fileName)}`;
  };
  ...
  const handleDeleteFile = async (fileId: number) => {
    ...
    try {
      const apiUrl = getApiUrl();
      const response = await fetch(`${apiUrl}/api/cpd/files/${fileId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleViewQuiz = async (quiz: any) => {
    try {
      const apiUrl = getApiUrl();
      const response = await fetch(`${apiUrl}/api/cpd/quizzes/${quiz.id}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const handleUpdateQuizScore = async (quizId: number, passingScore: number) => {
    try {
      const apiUrl = getApiUrl();
      const response = await fetch(`${apiUrl}/api/cpd/quizzes/${quizId}/passing-score`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleDeleteQuestion = async (questionId: number) => {
    ...
    try {
      const apiUrl = getApiUrl();
      const response = await fetch(`${apiUrl}/api/cpd/questions/${questionId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleAddQuestion = async (quizId: number) => {
    ...
    try {
      const apiUrl = getApiUrl();
      const response = await fetch(`${apiUrl}/api/cpd/quizzes/${quizId}/questions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleUploadFiles = async (topicId: number, files: FileList) => {
    ...
    try {
      const apiUrl = getApiUrl();
      const response = await fetch(`${apiUrl}/api/cpd/topics/${topicId}/upload-files`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        },
  ...
  const handleReplaceFile = async (fileId: number, file: File) => {
    ...
    try {
      const apiUrl = getApiUrl();
      const response = await fetch(`${apiUrl}/api/cpd/files/${fileId}/replace`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        },
  ...
  const handleSaveQuizEdit = async () => {
    ...
    try {
      ...
      const apiUrl = getApiUrl();
      await fetch(`${apiUrl}/api/cpd/quizzes/${viewingQuiz.quiz.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      const response = await fetch(`${apiUrl}/api/cpd/quizzes/${viewingQuiz.quiz.id}/update-gift`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  if (loading) {
    return (
      <ProtectedRoute allowedRoles={['Admin', 'Assessor', 'Operation Manager', 'Team Member']} userRole={userRole}>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div className="space-y-3">
              {topics.map((topic) => {
                ...
                return (
                  <div key={topic.id} className="space-y-2">
                    ...
                    {/* Selected Topic Content - Inline */}
                    {selectedTopic?.id === topic.id && (
                      <div className="mt-3 ml-4 p-6 bg-white rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="mb-6">
                ...
                {(selectedTopic as any).sections && (selectedTopic as any).sections.length > 0 && (
                  <div className="space-y-4">
                    {(selectedTopic as any).sections.map((section: any) => (
                ...
                {(!selectedTopic.files || selectedTopic.files.length === 0) && 
                 (!(selectedTopic as any).sections || (selectedTopic as any).sections.every((s: any) => !s.files || s.files.length === 0)) && (
                ...
                <iframe
                  src={
                    viewingFile.url.includes('cloudinary.com')
                      ? `${getApiUrl()}/api/cpd/proxy-pdf?url=${encodeURIComponent(viewingFile.url)}`
                  ...
                  allow="fullscreen"
                  style={{ border: 'none' }}
                  onLoad={() => { if (process.env.NODE_ENV === 'development') { console.log('[CPD] PDF loaded successfully'); } }}
                  onError={() => {}}
                />
