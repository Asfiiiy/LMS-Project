...
import ConsultationQuickActionCard from '@/app/components/ConsultationQuickActionCard';
import { CardSkeleton, StatSkeleton } from '@/app/components/ui/Skeleton';
...
const StudentDashboard = () => {
  ...
  // Qualification upgrade banner
  const [showUpgradeBanner, setShowUpgradeBanner] = useState(false);

  // Verification status banner ('pending' | 'rejected' | null)
  const [verificationStatus, setVerificationStatus] = useState<string | null>(null);

  // Consultation data for smart quick action card
  const [consultationBookings, setConsultationBookings] = useState<any[]>([]);
  const [consultationSlots, setConsultationSlots] = useState<any[]>([]);
  const [consultationByDate, setConsultationByDate] = useState<Record<string, any[]>>({});
  ...
  useEffect(() => {
    ...
    if (storedUserRaw) {
      ...
      const rawRole = storedUserRaw.role ?? null;
      const allowedStudentRoles = ['Student', 'ManagerStudent', 'InstituteStudent'];
      const normalizedRole = (rawRole && typeof rawRole === 'string'
        ? (allowedStudentRoles.includes(rawRole) ? rawRole : (rawRole.charAt(0).toUpperCase() + rawRole.slice(1).toLowerCase()))
      ...
      if (normalizedUser.id) {
        const fetchStudentData = async (studentId: number) => {
          ...
          try {
            ...
            // Set Qualification courses and normalize progress
            const rawQualificationCourses: QualificationCourse[] = qualificationCoursesData?.qualificationCourses ?? [];
            const normalizedQualificationCourses: QualificationCourse[] = rawQualificationCourses.map((c) => {
              const totalUnits = typeof c.total_units === 'number' ? c.total_units : 0;
              const completedUnits = typeof c.completed_units === 'number' ? c.completed_units : 0;
              const backendProgress = typeof c.progress === 'number' ? c.progress : 0;

              const computedProgress =
                totalUnits > 0
                  ? Math.round((Math.max(0, Math.min(completedUnits, totalUnits)) / totalUnits) * 100)
                  : backendProgress;
            ...
            setQualificationCourses(normalizedQualificationCourses);
          } catch {
  ...
  useEffect(() => {
    ...
    const allowed = ['Student', 'ManagerStudent', 'InstituteStudent'];
    if (!user.role || !allowed.includes(user.role)) return;
    let cancelled = false;
    ...
    Promise.all([
      ...
      if (cancelled) return;
      if (slotsRes?.success) {
        setConsultationSlots(slotsRes.slots || []);
        setConsultationByDate(slotsRes.byDate || {});
      }
      if (bookingsRes?.success) setConsultationBookings(bookingsRes.bookings || []);
    }).catch(() => {}).finally(() => {
  ...
  useEffect(() => {
    ...
    onboardingService.checkQualificationUpgrade()
      .then((res) => {
        if (res.success && res.upgrade_needed) {
    ...
    onboardingService.getStatus()
      .then(async (res) => {
        ...
        const s = res.status;
        const verified = s.admin_verified === true || (s.admin_verified as any) === 1;
        const requested = !!s.verification_requested_at;
        if (verified) {
          setVerificationStatus(null);
        } else if (requested) {
          try {
            const docsRes = await apiService.getMyDocuments();
            if (docsRes.success && docsRes.documents?.some((d: any) => d.status === 'rejected')) {
  ...
  const fetchConsultationData = () => {
    ...
    Promise.all([
      ...
      if (bookingsRes?.success) setConsultationBookings(bookingsRes.bookings || []);
    }).catch(() => {}).finally(() => setConsultationLoading(false));
  };
  ...
  const featuredCourse = useMemo(() => {
    const qualCoursesVisible = verificationStatus !== 'pending' && verificationStatus !== 'rejected';
    ...
    const inProgressQualification = qualCoursesVisible ? qualificationCourses.find(qual => qual.progress < 100) : undefined;
    ...
    if (qualCoursesVisible && qualificationCourses.length > 0) {
    ...
    if (courses.length > 0) {
      return { ...courses[0], type: 'regular' as const, instructor: 'Assessor' };
    }
    ...
    return null;
  }, [courses, cpdCourses, qualificationCourses, verificationStatus]);
  ...
  const filteredQualificationCourses = useMemo(() => {
    if (verificationStatus === 'pending' || verificationStatus === 'rejected') return [];
  ...
  }, [qualificationCourses, activeTab, verificationStatus]);
  ...
  const handleChatWithTutor = async () => {
    ...
    try {
      ...
      if (response.success && response.tutors && response.tutors.length > 0) {
        ...
        notify.infoAlert('No assessors found', 'You may not be enrolled in any courses yet.');
      }
    } catch {
      notify.errorAlert('Unable to load tutors', 'Please try again later.');
    } finally {
  ...
  const startChatWithTutor = async (tutorId: number) => {
    ...
    try {
      ...
      if (data.success && data.conversation) {
        ...
        notify.errorAlert('Unable to start chat', 'Please try again.');
        setLoadingChatWithTutor(false);
      }
    } catch {
      notify.errorAlert('Unable to start chat', 'Please try again.');
    ...
    <ProtectedRoute allowedRoles={['Student', 'ManagerStudent', 'InstituteStudent']} userRole={userRole} authReady={authReady}>
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-3 shadow-lg">
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
            <p className="text-sm font-medium text-center sm:text-left">
              You have been enrolled in a qualification course. Complete additional onboarding steps to get started.
            </p>
            <button
              onClick={async () => {
                try {
                  await onboardingService.startQualificationUpgrade();
                  router.push('/onboarding/qualification-level');
                } catch {
                  router.push('/onboarding/qualification-level');
                }
              }}
              className="px-4 py-1.5 bg-white text-blue-700 rounded-lg text-sm font-semibold hover:bg-blue-50 transition-colors whitespace-nowrap"
            >
              Complete Now
            </button>
          </div>
      ...
      {verificationStatus === 'pending' && (
        <div className="bg-gradient-to-r from-amber-500 to-orange-500 text-white px-4 py-3 shadow-lg">
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
            <p className="text-sm font-medium text-center sm:text-left">
              Your profile is under review by our admissions team. You&apos;ll have full access once verified.
            </p>
            <button
              onClick={() => router.push('/onboarding/verification-pending')}
              className="px-4 py-1.5 bg-white text-amber-700 rounded-lg text-sm font-semibold hover:bg-amber-50 transition-colors whitespace-nowrap"
            >
              View Status
            </button>
          </div>
      ...
      {verificationStatus === 'rejected' && (
        <div className="bg-gradient-to-r from-red-600 to-rose-600 text-white px-4 py-3 shadow-lg">
          <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
            <p className="text-sm font-medium text-center sm:text-left">
              Some of your documents were rejected. Please resubmit them to continue the verification process.
            </p>
            <button
              onClick={() => router.push('/onboarding/resubmit')}
              className="px-4 py-1.5 bg-white text-red-700 rounded-lg text-sm font-semibold hover:bg-red-50 transition-colors whitespace-nowrap"
            >
              Resubmit Documents
            </button>
          </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
        ...
        <div className="relative max-w-7xl mx-auto p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6 md:space-y-8">
          {/* Dashboard Content - always shown; profile completion is no longer required for access */}
          {(
            <div>
              {/* Header Section */}
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mt-6 sm:mt-8 mb-6 sm:mb-8">
            {isLoading ? (
              <>
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="bg-white/80 backdrop-blur-xl rounded-xl sm:rounded-2xl shadow-2xl border border-white/20 p-4 sm:p-5 md:p-6">
                    <StatSkeleton />
                  </div>
                ))}
              </>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-6 text-blue-100 mb-4 sm:mb-6 text-xs sm:text-sm">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="truncate">Assessor</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <svg className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        ...
                        <div className="mb-4 sm:mb-6">
                          <div className="flex justify-between text-blue-100 mb-2 sm:mb-3 text-xs sm:text-sm">
                            ...
                            <span className="font-black">{clampProgress(featuredCourse.progress ?? 0)}%</span>
                          </div>
                          <div className="w-full bg-white/30 rounded-full h-2 sm:h-3">
                            <div 
                              ...
                              style={{ width: `${clampProgress(featuredCourse.progress ?? 0)}%` }}
                            ></div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="self-center sm:self-start sm:ml-4 md:ml-8 flex-shrink-0">
                        <div className="w-16 h-16 sm:w-18 sm:h-18 md:w-20 md:h-20 bg-white/20 rounded-xl sm:rounded-2xl flex items-center justify-center backdrop-blur-sm border border-white/30">
                          <div className="text-center">
                            <div className="text-xl sm:text-2xl font-black">{clampProgress(featuredCourse.progress ?? 0)}%</div>
              ...
              <div className="bg-white/80 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-2xl border border-white/20 p-4 sm:p-6 md:p-8">
                ...
                {isLoading && courses.length === 0 && cpdCourses.length === 0 && qualificationCourses.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    {/* CPD Courses */}
                    {filteredCPDCourses.map((cpdCourse, index) => (
                      <div key={`cpd-${cpdCourse.course_id}-${index}`} className="group">
                    ...
                    {filteredQualificationCourses.map((qualCourse, index) => (
                      <div key={`qual-${qualCourse.course_id}-${index}`} className="group">
                        <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 shadow-lg border border-blue-200 hover:shadow-2xl transform hover:scale-[1.02] sm:hover:scale-105 transition-all duration-500 hover:border-blue-400">
                          ...
                          <div className="mb-3 sm:mb-4">
                            <div className="flex justify-between text-xs sm:text-sm text-gray-600 mb-1.5 sm:mb-2">
                              <span>Progress</span>
                              <span className="font-semibold">{clampProgress(qualCourse.progress ?? 0)}%</span>
                            </div>
                            <div className="w-full bg-gray-200 rounded-full h-1.5 sm:h-2">
                              <div 
                                className={`h-1.5 sm:h-2 rounded-full transition-all duration-1000 ${
                                  clampProgress(qualCourse.progress ?? 0) === 100
                                    ? 'bg-gradient-to-r from-green-400 to-emerald-500' 
                                    : 'bg-gradient-to-r from-blue-400 to-cyan-500'
                                }`}
                                style={{ width: `${clampProgress(qualCourse.progress ?? 0)}%` }}
                              ></div>
                            </div>
                          </div>
            ...
            <div className="space-y-4 sm:space-y-6 md:space-y-8">
              ...
              <div className="bg-white/80 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-2xl border border-white/20 p-4 sm:p-5 md:p-6">
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <ConsultationQuickActionCard
                    bookings={consultationBookings}
                    slots={consultationSlots}
                    byDate={consultationByDate}
                  ...
                  {[
                    ...
                    { icon: '💳', title: 'Payment Plan', desc: 'View your payment schedule', color: 'from-[#11CCEF] to-cyan-500', link: '/dashboard/student/profile?tab=payments', key: 'payment-plan' },
                  ].map((action, index) => (
                    <button
                      key={index}
                      onClick={() => {
                    ...
                    <>
                      ...
                      <div className="space-y-3 sm:space-y-4">
                        {displayedDeadlines.map((deadline, index) => (
                          <div key={`deadline-${deadline.id}-${deadline.courseId}-${index}`} className="bg-white/20 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-white/30">
          ...
          {showTutorModal && (
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl max-w-md w-full p-4 sm:p-6 md:p-8 transform transition-all max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                <h3 className="text-xl sm:text-2xl font-black text-gray-900">Select an Assessor</h3>
                <button
              ...
              <p className="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base">Choose an assessor from your enrolled courses to start chatting:</p>
