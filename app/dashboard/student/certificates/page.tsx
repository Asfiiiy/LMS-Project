...
import { CertificatesPageSkeleton } from '@/app/components/ui/Skeleton';
...
type UserRole = 'Admin' | 'Assessor' | 'Manager' | 'Student' | 'Moderator' | 'Operation Manager' | 'Accounts Manager' | 'Administrative Manager' | 'ManagerStudent' | 'InstituteStudent' | null;
...
function CertificatesPage() {
  ...
  const fetchData = async (studentId: number) => {
    try {
      ...
      // Fetch all 4 in parallel — partial data shown if some fail
      const [claimsRes, deliveredRes, cpdRes, qualRes] = await Promise.allSettled([
        apiService.getMyMyCertificateClaims(),
        apiService.getMyDeliveredCertificates(),
      ...
      if (deliveredRes.status === 'fulfilled' && deliveredRes.value?.success) {
        setDeliveredCerts(deliveredRes.value.certificates ?? []);
      } else if (deliveredRes.status === 'rejected') {
      }

      if (cpdRes.status === 'fulfilled' && cpdRes.value?.success) {
        const completedCPD = (cpdRes.value.cpdCourses ?? []).filter((c: any) =>
      ...
      } else if (cpdRes.status === 'rejected') {
      }

      if (qualRes.status === 'fulfilled' && qualRes.value?.success) {
        const completedQual = (qualRes.value.qualificationCourses ?? []).filter((c: any) =>
      ...
      } else if (qualRes.status === 'rejected') {
      }

      const anyFailed = [claimsRes, deliveredRes, cpdRes, qualRes].some(r => r.status === 'rejected');
      if (anyFailed) {
        notify.warning('Some data may be incomplete. Please refresh to retry.');
      }
    } catch {
