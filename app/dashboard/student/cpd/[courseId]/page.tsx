  ...
  const loadCourseData = async (userId: number) => {
    try {
      ...
      if (data.success) {
        ...
        setFaq(data.faq);
        // Sort topics by order_index to ensure correct order (safety measure)
        const sortedTopics = [...(data.topics || [])].sort((a, b) => {
          const aOrder = a.order_index ?? a.topic_number ?? 0;
          const bOrder = b.order_index ?? b.topic_number ?? 0;
        ...
        const firstUnlocked = sortedTopics.find((t: CPDTopic) => t.progress.is_unlocked === 1);
        if (firstUnlocked) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div className="space-y-3">
              {topics.map((topic) => {
                ...
                return (
                  <div key={topic.id} className="space-y-2">
                    <div
                      onClick={() => topic.progress.is_unlocked ? setSelectedTopic(selectedTopic?.id === topic.id ? null : topic) : null}
                    ...
                    {selectedTopic?.id === topic.id && topic.progress.is_unlocked && (
              ...
              {/* Topic Files - Organized by Sections */}
              <div className="mb-6">
                ...
                {(!selectedTopic.files || selectedTopic.files.length === 0) && 
                 (!(selectedTopic as any).sections || (selectedTopic as any).sections.every((s: any) => !s.files || s.files.length === 0)) && (
                  <p className="text-gray-500 text-sm">No files available for this topic</p>
                )}
              </div>
