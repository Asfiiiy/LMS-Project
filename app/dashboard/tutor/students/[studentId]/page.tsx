  ...
  const fetchData = async () => {
    try {
      ...
      const [profileResponse, paymentsResponse] = await Promise.all([
        apiService.getTutorStudentProfileById(studentId),
        tutorId ? apiService.getTutorPayments(tutorId) : Promise.resolve({ success: false, installments: [] })
      ]);
      ...
      if (paymentsResponse?.success) {
        ...
        const studentPayments = (paymentsResponse.installments || []).filter(
          (inst: any) => inst.student_id === studentId
        );
