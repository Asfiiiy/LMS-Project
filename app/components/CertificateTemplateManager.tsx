...
const CertificateTemplateManager: React.FC = () => {
  ...
  // Handle preview - opens in new tab with authentication
  const handlePreview = async (templateId: number, templateName: string) => {
    try {
      const blob = await apiService.downloadCertificateTemplateFile(templateId);
      const url = window.URL.createObjectURL(blob);
      const newWindow = window.open(url, '_blank');
      if (!newWindow) {
        showToast('Please allow popups to preview', 'error');
      }
      // Clean up the URL after a delay
      setTimeout(() => window.URL.revokeObjectURL(url), 100);
    } catch (error: any) {
      showToast(error.message || 'Failed to preview template', 'error');
    }
  };

  // Handle download - downloads with authentication
              ...
              <div key={template.id} className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border-3 border-green-500">
                ...
                <div className="p-4 space-y-2.5">
                  ...
                  <div className="grid grid-cols-3 gap-2 pt-2">
                    ...
                    <button
                      onClick={() => handlePreview(template.id, template.template_name)}
