...
interface CourseManagementProps {
  ...
  useTicketsApi?: boolean;
  basePathForView?: string;
}

const CourseManagement = ({ filterType = 'all', showControls = true, useTicketsApi = false, basePathForView }: CourseManagementProps = {}) => {
  const [courseType, setCourseType] = useState<'cpd' | 'qualification' | 'categories'>('cpd');
  ...
  const getViewBasePath = () => {
    if (basePathForView) return basePathForView;
    const user = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('lms-user') || '{}') : {};
    const role = (user?.role || 'admin').toString().toLowerCase().replace(/\s+/g, '_');
    ...
    try {
      ...
      if (categoriesData.success) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('✅ Categories refreshed:', categoriesData.categories.length);
        }
      }
    } catch (error) {
      // no-op
  ...
  const fetchData = async () => {
    try {
      ...
      const [coursesData, categoriesData] = await Promise.all([
        useTicketsApi ? apiService.getTicketsCourses() : apiService.getCourses(),
        useTicketsApi ? apiService.getTicketsCourseCategories() : apiService.getCourseCategories()
      ]);
  ...
  const handleCreateCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showSweetAlert({
          title: 'Error',
          text: data.message || 'Error creating category',
          icon: 'error',
          confirmButtonText: 'OK'
        });
      }
  ...
  const handleDeleteCategory = async (categoryId: number, categoryName: string) => {
    showSweetAlert({
      ...
      text: `Are you sure you want to delete the category "${categoryName}"? This will also delete all sub-categories under it.`,
      icon: 'warning',
      ...
      onConfirm: async () => {
        try {
          const data = await apiService.deleteCourseCategory(categoryId);
          if (data.success) {
            await fetchCategories();
            showSweetAlert({
              title: 'Success!',
  ...
  const handleCreateSubCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        // Refresh categories to update sub-category dropdown
        await fetchCategories();
        ...
        showSweetAlert({
          title: 'Error',
          text: data.message || 'Error creating sub-category',
          icon: 'error',
          confirmButtonText: 'OK'
        });
      }
  ...
  const handleDeleteSubCategory = async (subCategoryId: number, subCategoryName: string) => {
    showSweetAlert({
      ...
      text: `Are you sure you want to delete the sub-category "${subCategoryName}"?`,
      icon: 'warning',
      showConfirmButton: true,
      confirmButtonText: 'Yes, Delete',
      showCancelButton: true,
      ...
      onConfirm: async () => {
        try {
          const data = await apiService.deleteSubCategory(subCategoryId);
          if (data.success) {
            await fetchCategories();
            showSweetAlert({
              title: 'Success!',
            ...
            showSweetAlert({
              title: 'Error',
              text: data.message || 'Error deleting sub-category',
              icon: 'error',
              confirmButtonText: 'OK'
            });
          }
          ...
          // Extract error message from error object
          const errorMessage = error?.message || 'Error deleting sub-category';
          showSweetAlert({
            title: 'Error',
            text: errorMessage,
            icon: 'error',
  ...
  const filteredCourses = useMemo(() => {
    let filtered = courses.filter(course => {
      ...
      // Filter by course type tab (cpd or qualification) - only if filterType is 'all' and not showing categories
      const matchesCourseType = courseType === 'categories' 
        ? false // Hide all courses when showing categories
        : filterType === 'all'
  ...
  return (
    <div className="space-y-6">
        ...
        <>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <div className="flex items-center gap-4 mb-4">
              ...
              <button
                onClick={() => setCourseType('categories')}
                className={`px-6 py-3 rounded-lg font-medium transition-all ${
      ...
      {/* Categories List - Show all categories with delete option (only when "All Categories" is selected) */}
      {showControls && courseType === 'categories' && (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 mb-4">
          <h3 className="text-lg font-semibold mb-4">Categories & Sub-Categories</h3>
          <div className="space-y-4">
            {categories.length === 0 ? (
              <p className="text-gray-500 text-sm">No categories yet. Create one using the "Add Category" button above.</p>
            ) : (
                ...
                <div key={category.id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    ...
                    <button
                      ...
                      className="ml-4 px-3 py-1.5 bg-red-600 text-white text-sm rounded-lg hover:bg-red-700 transition-colors"
                      title="Delete category"
                    >
                      🗑️ Delete
                    </button>
      ...
      {/* Filters - Show in both tabs (hide when showing categories) */}
      {courseType !== 'categories' && (
      ...
      {/* Courses Grid - Hide when showing categories */}
      {courseType !== 'categories' && (
            ...
            <div className="px-6 py-3 bg-gray-50 border-t border-gray-200">
              <div className="flex justify-between items-center">
                {course.course_type === 'cpd' ? (
                  useTicketsApi ? (
                    <button
                      onClick={() => window.location.href = `${getViewBasePath()}/cpd/${course.id}/view`}
                    ...
                    <button
                      onClick={() => window.location.href = `${getViewBasePath()}/cpd/${course.id}/manage`}
                      className="text-[#11CCEF] hover:text-[#0daed9] text-sm font-medium cursor-pointer"
                    >
                      📚 Manage Course
                    </button>
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => window.location.href = `${getViewBasePath()}/qualification/${course.id}/view`}
                    ...
                    {!useTicketsApi && (
                      <>
                        ...
                        <button
                          onClick={() => window.location.href = `${getViewBasePath()}/qualification/${course.id}/manage`}
                ...
                <div className="flex space-x-2">
                  {course.course_type === 'cpd' ? (
                    <>
                      <button
                        onClick={() => {
                          window.location.href = `${getViewBasePath()}/cpd/${course.id}/view`;
                        }}
                    ...
                    <>
                      <button
                        onClick={() => {
                          window.location.href = `${getViewBasePath()}/qualification/${course.id}/manage`;
                        }}
      ...
      {/* Pagination - Show in both tabs (hide when showing categories) */}
      {courseType !== 'categories' && ((showControls && totalPages > 1) || (!showControls && totalCoursesTotalPages > 1)) && (
