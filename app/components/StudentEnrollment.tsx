...
interface CourseSummary {
  ...
  course_type?: 'cpd' | 'qualification' | null;
}
...
const StudentEnrollment = ({ role, userId }: StudentEnrollmentProps) => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      setFeedback(null);
      try {
        const [coursesResponse, studentsResponse, tutorsResponse] = await Promise.all([
          role === 'Assessor' && userId
  ...
  const handleEnrollStudent = async (studentId: number) => {
    ...
    try {
      ...
      if (role === 'Assessor' && userId) {
        enrollmentResponse = await apiService.enrollStudents([studentId], selectedCourseId, userId, selectedTutorId || undefined);
        // Refetch enrollments to update the UI
        const response = await apiService.getTutorCourseEnrollments(userId, selectedCourseId);
        setEnrollments(response?.enrollments ?? response?.students ?? []);
      } else {
        enrollmentResponse = await apiService.enrollStudents([studentId], selectedCourseId, undefined, selectedTutorId || undefined);
        // Refetch enrollments to update the UI
        const response = await apiService.getCourseEnrollments(selectedCourseId);
        setEnrollments(response?.enrollments ?? []);
      }

      // Reset pagination to show updated lists
      setAvailablePage(1);
      setEnrolledPage(1);

      // For CPD courses: enroll directly without setup page (deadlines/payments handled differently)
      // For Qualification courses: redirect to enrollment setup page for deadlines and payments
      const selectedCourse = courses.find((c) => c.id === selectedCourseId);
      const isCpdCourse = selectedCourse?.course_type === 'cpd';

      if (enrollmentResponse?.studentIds && enrollmentResponse?.studentIds.length > 0 && !isCpdCourse) {
        const firstStudentId = enrollmentResponse.studentIds[0];
        const dashboardPath = role === 'Assessor' ? 'tutor' : 'admin';
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600">Select a course, assign a tutor, and manage student enrollments.</p>
            </div>
          ...
          {/* Tutor Selection Row */}
          <div className="flex flex-col md:flex-row md:items-center gap-4 p-4 bg-purple-50 border border-purple-200 rounded-lg">
            <div className="flex-1">
              <label className="text-sm font-medium text-gray-700 mb-1 block">
                👨‍🏫 Assign Tutor (Optional)
              </label>
              <p className="text-xs text-gray-500">
                Select a main or sub tutor to handle this student's work and communications for this course
              </p>
            </div>
            <div className="flex items-center gap-3 md:min-w-[350px]">
              <select
                ...
                onChange={(event) => setSelectedTutorId(event.target.value ? Number(event.target.value) : null)}
