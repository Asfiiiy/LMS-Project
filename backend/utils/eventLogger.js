...
const roleIdToName = {
  ...
  6: 'operation_manager',
  7: 'accounts_manager',
  8: 'administrative_manager',
  9: 'admission_manager',
  10: 'team_member',
  11: 'certificate_manager',
  12: 'claim_manager',
  13: 'manager_student',
  14: 'institute_student',
  15: 'consultation_manager'
};
...
async function logSystemEvent({ 
  ...
  try {
    ...
    // Check if this is an AI token request
    let aiTokenName = null;
    if (req?.aiToken) {
      role = 'ai_agent';
      aiTokenName = req.aiToken.name || 'Unknown AI Agent';
      userId = req.aiToken.created_by || userId; // Use the creator's ID for tracking
    }
    ...
    let userName = null;
    if (aiTokenName) {
      // For AI tokens, use the token name as the user name
      userName = aiTokenName;
    } else if (userId && !role) {
      try {
    ...
    if (userName && description) {
      ...
      enhancedDescription = description
        .replace(/^(Assessor|Admin|User|Student|Manager|Moderator)\s+/, `${userName} `)
        .replace(/^User #\d+\s+/, `${userName} `);
    }

    // Check if user_name column exists (for backward compatibility)
    ...
    try {
      ...
      hasUserNameColumn = columns.length > 0;
    } catch (err) {
      // Silently fail - will use old format
    }

    // Insert into system_logs (with backward compatibility)
    if (hasUserNameColumn) {
      await pool.execute(
        `INSERT INTO system_logs
          (user_id, user_name, course_id, student_id, service, role, action, description, ip_address, country_code, country_name, user_agent, endpoint, method, request_body, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
        [userId, userName, courseId, studentId, service, role, action, enhancedDescription, ip_address, countryCode, countryName, user_agent, endpoint, method, request_body]
      );
    } else {
      // Old format without user_name column
      await pool.execute(
        `INSERT INTO system_logs
