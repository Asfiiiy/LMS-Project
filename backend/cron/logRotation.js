...
async function executeWithRetry(connection, sql, params, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      return await connection.execute(sql, params);
    } catch (err) {
      if (err.code === 'ER_LOCK_DEADLOCK' && attempt < maxRetries) {
        console.warn(`[LogRotation] Deadlock on attempt ${attempt}, retrying...`);
  ...
  cron.schedule('0 3 * * *', async () => {
    try {
      ...
      try {
        ...
        const [archiveResult] = await executeWithRetry(
          connection,
