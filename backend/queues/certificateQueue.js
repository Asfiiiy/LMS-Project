...
// Load .env file from backend directory (same as server.js)
...
// Remove quotes from password if present (same as config/redis.js)
const redisPassword = (process.env.REDIS_PASSWORD || '').replace(/^["']|["']$/g, '');

if (process.env.REDIS_HOST && redisPassword && process.env.REDIS_PORT) {
  // Use same config as working Redis connection
  redisConfig = {
    host: process.env.REDIS_HOST,
    port: parseInt(process.env.REDIS_PORT) || 6379,
    password: redisPassword, // ✅ Fixed: Use password without quotes
  ...
  redisConfig = {
    ...
    password: url.password ? url.password.replace(/^["']|["']$/g, '') : undefined, // ✅ Fixed: Strip quotes
