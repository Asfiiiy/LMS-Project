...
router.get('/', auth, permit('Admin'), async (req, res) => {
  try {
    ...
    // For CSV export allow larger limit (download all logs)
    const maxLimit = format === 'csv' ? 50000 : 500;
    const finalLimit = Math.max(1, Math.min(maxLimit, parseInt(limit, 10) || 50));
    ...
    // Search by student/user name or email (partial match)
    if (search_user && String(search_user).trim()) {
      const term = `%${String(search_user).trim()}%`;
      whereClauses.push('sl.user_id IN (SELECT id FROM users WHERE name LIKE ? OR email LIKE ?)');
    ...
    // For AI agents, prefer sl.user_name (contains AI token name) over u.name if column exists
    let query = `SELECT 
        sl.id,
        sl.user_id,
        ...
        COALESCE(
          CASE 
            WHEN sl.role = 'ai_token' THEN 'ai_agent'
            ELSE sl.role
          END,
          CASE 
    ...
    if (format === 'csv') {
      ...
      for (const row of rows) {
        ...
        try {
          if (row.request_body) {
            const body = typeof row.request_body === 'string' ? JSON.parse(row.request_body) : row.request_body;
            if (body) {
              file_name = body.file_name || body.fileName || '';
              duration_seconds = body.duration_seconds != null ? String(body.duration_seconds) : (body.duration != null ? String(body.duration) : '');
              opened_at = body.opened_at || body.openedAt || '';
              closed_at = body.closed_at || body.closedAt || '';
              unit_id = body.unit_id != null ? String(body.unit_id) : (body.unitId != null ? String(body.unitId) : '');
            }
          }
        } catch (e) {
          // ignore parse errors
...
// Query params (optional): same as GET – delete only logs matching filters.
// If no filters: deletes all logs. Use with caution.
// =============================================
router.delete('/', auth, permit('Admin'), async (req, res) => {
  try {
    ...
    if (studentId) {
      ...
      whereClauses.push('(student_id = ? OR user_id = ?)');
      params.push(sid, sid);
    }
    if (search_user && String(search_user).trim()) {
      const term = `%${String(search_user).trim()}%`;
      whereClauses.push('user_id IN (SELECT id FROM users WHERE name LIKE ? OR email LIKE ?)');
      params.push(term, term);
    }

    const whereSQL = whereClauses.length ? 'WHERE ' + whereClauses.join(' AND ') : '';
    const [result] = await pool.execute(
      `DELETE FROM system_logs ${whereSQL}`,
      params
    );
    const deletedCount = result.affectedRows || 0;

    logger.info({ deletedCount, filters: req.query }, '[LOGS] Admin deleted logs');
    res.json({ success: true, deleted: deletedCount, message: `Deleted ${deletedCount} log(s).` });
  } catch (error) {
    logger.error({ error: error.message, stack: error.stack }, '[LOGS] Error deleting logs');
    res.status(500).json({
      success: false,
...
// =============================================
// GET /api/admin/logs/active-users
// Get currently active/online users with their details
// Query params:
//   page (default 1)
//   limit (default 20)
// =============================================
router.get('/active-users', auth, permit('Admin'), async (req, res) => {
  try {
    const { getOnlineUsersFromRedis } = require('../socket');
    const onlineUsersData = await getOnlineUsersFromRedis();
    ...
    const [users] = await pool.query(
      ...
      ORDER BY u.name ASC
      LIMIT ? OFFSET ?`,
      [userIdInts, limit, offset]
    );

    const enrichedUsers = users.map(user => {
      const socketData = onlineMap[user.id];
      return {
        ...user,
        status: 'online',
        lastSeen: socketData?.lastSeen || new Date().toISOString(),
        connectedAt: socketData?.connectedAt || null
      };
    });

    const total = userIdInts.length;
    ...
    logger.error({ error: err.message }, '[LOGS] Error fetching active users');
    res.status(500).json({ success: false, message: 'Error fetching active users', error: err.message });
  }
});
