...
router.post('/assignments/:assignmentId/submit', async (req, res) => {
  try {
    ...
    const [studentInfo] = await pool.execute('SELECT name, assigned_tutor_id FROM users WHERE id = ?', [studentId]);
    ...
    if (studentInfo.length > 0 && studentInfo[0].assigned_tutor_id) {
      ...
      console.log(`[Assignment Submit] Student ${studentId} has assigned tutor ${studentInfo[0].assigned_tutor_id}`);
    } else {
      ...
      tutors = mainTutors;
      console.log(`[Assignment Submit] Student ${studentId} has no assigned tutor, notifying ${tutors.length} main tutors`);
    }
