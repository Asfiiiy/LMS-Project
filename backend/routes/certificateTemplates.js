...
router.get('/:id/download', auth, async (req, res) => {
  try {
    ...
    let normalizedPath = template.template_path.replace(/\\/g, '/');
    ...
    if (path.isAbsolute(normalizedPath)) {
      ...
      // Relative path from backend root (e.g., templates/active/filename.docx)
      filePath = path.join(__dirname, '..', normalizedPath);
    } else {
      // Just filename - look in TEMPLATES_DIR
    ...
    console.log(`[Template Download] Template ID: ${template.id}`);
    console.log(`[Template Download] Template path from DB: ${template.template_path}`);
    console.log(`[Template Download] Resolved file path: ${filePath}`);
    console.log(`[Template Download] File exists: ${fs.existsSync(filePath)}`);
    
    if (!fs.existsSync(filePath)) {
      // Try alternative: check if it's just the filename in TEMPLATES_DIR
      const fileName = path.basename(template.template_path);
      const altPath = path.join(TEMPLATES_DIR, fileName);
      console.log(`[Template Download] Trying alternative path: ${altPath}`);
    ...
    res.status(500).json({ success: false, message: 'Error downloading template', error: error.message });
  }
});
