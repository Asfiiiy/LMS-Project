  ...
  try {
    ...
    if (hasRelatedUserId) {
      query = `SELECT 
        ...
        COALESCE(sp.profile_picture, stp.profile_picture) as related_user_avatar,
...
const optionalAuth = async (req, res, next) => {
  ...
  try {
    ...
    if ((req.method !== 'GET' || process.env.DEBUG_AUTH === 'true') && process.env.NODE_ENV === 'development') {
...
// =====================================================
// STATS (lightweight — no post data)
// =====================================================
router.get('/stats', optionalAuth, cacheMiddleware(300, (req) => {
  const userId = req.user?.id || 'anonymous';
  return `cache:forum:stats:${userId}`;
}), async (req, res) => {
  try {
    const [statsRows] = await pool.execute(
      `SELECT 
        (SELECT COUNT(*) FROM forum_posts) as totalPosts,
        (SELECT COUNT(DISTINCT author_id) FROM forum_posts) as totalMembers`
    );
    const totalPosts = parseInt(statsRows[0]?.totalPosts || 0, 10);
    const totalMembers = parseInt(statsRows[0]?.totalMembers || 0, 10);
    ...
    try {
      ...
      if (hasReactionType) {
        let trendingRows;
        try {
          [trendingRows] = await pool.execute(
            `SELECT fp.id, fp.title, COUNT(fpl.id) as reaction_count
             FROM forum_posts fp
             LEFT JOIN forum_post_likes fpl ON fp.id = fpl.post_id
             GROUP BY fp.id, fp.title
             ORDER BY reaction_count DESC, fp.last_activity_at DESC
             LIMIT 5`
          );
        } catch (e) {
          [trendingRows] = await pool.execute(
            `SELECT id, title FROM forum_posts ORDER BY created_at DESC LIMIT 5`
          );
        }
        trending = (trendingRows || []).map(r => ({
          id: r.id,
          title: r.title || '',
          count: parseInt(r.reaction_count || 0, 10),
          likes_count: parseInt(r.reaction_count || 0, 10),
          comments_count: 0
        }));
      } else {
      ...
      console.error('[Forum] Error fetching trending:', err);
    }
    ...
    try {
      ...
      if (colNames.includes('comments_count')) {
        const [cc] = await pool.execute(
          'SELECT COALESCE(SUM(comments_count), 0) as total FROM forum_posts'
        );
        totalComments = parseInt(cc[0]?.total || 0, 10);
      } else {
        const [cc] = await pool.execute('SELECT COUNT(*) as total FROM forum_comments');
        totalComments = parseInt(cc[0]?.total || 0, 10);
      }
      if (colNames.includes('last_activity_at')) {
        const [at] = await pool.execute(
          `SELECT COUNT(*) as total FROM forum_posts 
           WHERE DATE(last_activity_at) = CURDATE()`
        );
        activeToday = parseInt(at[0]?.total || 0, 10);
      } else {
        const [at] = await pool.execute(
          `SELECT COUNT(*) as total FROM forum_posts WHERE DATE(created_at) = CURDATE()`
        );
        activeToday = parseInt(at[0]?.total || 0, 10);
      }
    } catch (err) {
      console.error('[Forum] Error fetching comment/active stats:', err);
    }
    ...
    if (userId) {
      try {
        const [userRows] = await pool.execute(
          `SELECT 
            (SELECT COUNT(*) FROM forum_posts WHERE author_id = ?) as userPosts,
            (SELECT COUNT(*) FROM forum_comments WHERE author_id = ?) as userComments,
            (SELECT COUNT(*) FROM forum_post_likes WHERE user_id = ?) as userLikes`,
          [userId, userId, userId]
        );
        payload.userStats = {
          userPosts: parseInt(userRows[0]?.userPosts || 0, 10),
          userComments: parseInt(userRows[0]?.userComments || 0, 10),
          userLikes: parseInt(userRows[0]?.userLikes || 0, 10)
        };
      } catch (err) {
        console.error('[Forum] Error fetching user stats:', err);
      }
    }
...
router.get('/posts', optionalAuth, cacheMiddleware(60, (req) => {
  ...
  try {
    ...
    // Get profile picture from both student_profiles and staff_profiles
    const profileField = 'COALESCE(sp.profile_picture, stp.profile_picture) as author_avatar,';
...
router.get('/posts/:postId', optionalAuth, cacheMiddleware(60, (req) => {
  ...
  try {
    ...
    // Get profile picture from both student_profiles and staff_profiles for comments
    const commentProfileField = 'COALESCE(sp.profile_picture, stp.profile_picture) as author_avatar,';
...
router.post('/posts', auth, async (req, res) => {
  try {
    ...
    const allowedRoles = ['Student', 'Moderator', 'Admin', 'Assessor', 'ManagerStudent', 'InstituteStudent'];
    ...
    try {
      [result] = await pool.execute(
        `INSERT INTO forum_posts (category_id, author_id, title, content, last_activity_at)
         VALUES (?, ?, ?, ?, NOW())`,
        [category_id || null, authorId, sanitizedTitle, sanitizedContent]
      );
    } catch (err) {
      if (err.code === 'ER_BAD_FIELD_ERROR') {
        ...
        [result] = await pool.execute(
          `INSERT INTO forum_posts (forum_id, author_id, content, created_at)
           VALUES (?, ?, ?, NOW())`,
          [category_id || null, authorId, sanitizedContent]
        );
      } else {
        throw err;
      }
    ...
    // If post is created by Tutor or Admin, notify all students
    // Check both role string and role_id to handle different auth methods
    const isTutorOrAdmin = userRole === 'Assessor' || userRole === 'Admin' || 
                          (req.user.role_id === 2) || (req.user.role_id === 1);
    
    if (process.env.NODE_ENV === 'development') {
      console.log(`[Forum] Post created by user ${authorId}, role: ${userRole}, role_id: ${req.user.role_id}, isTutorOrAdmin: ${isTutorOrAdmin}`);
    }
    ...
    if (isTutorOrAdmin) {
      setImmediate(async () => {
        try {
          ...
          const authorName = authorInfo[0]?.name || (userRole === 'Assessor' ? 'An Assessor' : 'An Admin');

          // Fetch all students (join with roles table to get role names)
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log(`[Forum] Found ${students.length} students to notify about post ${postId}`);
          }
          ...
          for (const student of students) {
            try {
              const notificationId = await createNotification(req, {
                userId: student.id,
                type: 'forum_post',
              ...
              if (notificationId) {
                ...
                errorCount++;
                if (process.env.NODE_ENV === 'development') {
                  console.error(`[Forum] createNotification returned null for student ${student.id}`);
                }
              }
            } catch (err) {
              errorCount++;
              if (process.env.NODE_ENV === 'development') {
                console.error(`[Forum] Error creating notification for student ${student.id}:`, err);
              }
            }
          }
          ...
          if (process.env.NODE_ENV === 'development') {
            if (errorCount > 0) {
              console.error(`[Forum] Failed to create ${errorCount} notifications out of ${students.length} students`);
            }
            console.log(`[Forum] Successfully notified ${notificationCount} out of ${students.length} students about new post by ${userRole}`);
          }
        } catch (error) {
          console.error('[Forum] Error notifying students about new post:', error);
          // Don't fail the request if notification fails
        }
      });
    } else {
      if (process.env.NODE_ENV === 'development') {
        console.log(`[Forum] Post created by ${userRole} - no notifications sent (only Tutor/Admin posts trigger notifications)`);
      }
    }
  ...
  try {
    ...
    const [result] = await pool.execute(
      `INSERT INTO forum_comments (post_id, author_id, parent_comment_id, content)
       VALUES (?, ?, ?, ?)`,
      [postId, authorId, parent_comment_id || null, sanitizedContent]
    );
